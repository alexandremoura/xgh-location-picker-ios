...
final class LocationPickerViewController: UIViewController {
	...
	public var searchBarPlaceholder = "Pesquisar um endereço"
	public var searchHistoryLabel = "Histórico de busca"
    public var selectButtonTitle = " Selecionar "
    ...
    lazy var locationButton: Button = {
        ...
        if #available(iOS 13.0, *) {
            $0.setImage(UIImage(systemName: "location.fill"), for: UIControl.State())
        } else {
...
class CustomAnnotationView : MKPinAnnotationView{
    ...
    var delegate: LocationPickerViewController! = nil
    let detailView = UIView(frame: CGRect(x: 0, y: -75, width: defaultWidth, height: 70))
    let detailLabel = UILabel(frame: CGRect(x: 0, y: 0, width: defaultWidth, height: 30))
    public let detailButton = UIButton(frame: CGRect(x: 0, y: 30, width: defaultWidth, height: 30))
    
    override func setSelected(_ selected: Bool, animated: Bool) {
        super.setSelected(selected, animated: animated)
        
        if selected {
            detailLabel.text = annotation?.title as? String ?? ""
            let width = detailLabel.textRect(forBounds: CGRect(x: 0, y: 0, width: Int.max, height: 30), limitedToNumberOfLines: 1).width
            let bestWidth = max(width, CustomAnnotationView.defaultWidth)

            detailLabel.frame.size = CGSize(width: bestWidth+20, height: 30.0)
            detailView.frame.size.width = detailLabel.frame.size.width
            ...
            detailButton.backgroundColor = .systemBlue
            
            detailButton.setTitle(" Selecionar ", for: UIControl.State())
            
            detailView.center.x = 0.5 * self.frame.size.width
            detailButton.center.x = 0.5 * detailView.frame.size.width
    ...
    override func point(inside point: CGPoint, with event: UIEvent?) -> Bool {
        ...
        if (!isInside) {
            for subview in subviews {
                ...
                if (isInside) {
                    self.delegate.detailButtonTapped()
                    break
                }
            }
        }
...
extension LocationPickerViewController: MKMapViewDelegate {
    public func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
		...
		let pin = CustomAnnotationView(annotation: annotation, reuseIdentifier: "annotation")
        pin.delegate = self
		pin.pinTintColor = UIColor(hex: 0x007AFF) //UIColor(hex: 0xFF2DC6)
        
//        pin.detailButton.addTarget(self, action: #selector(detailButtonTapped), for: .touchUpInside)
...
//		pin.detailCalloutAccessoryView = selectLocationButton()
...
//        let button = UIButton(frame: CGRect(x: 50, y: -20, width: 100, height: 30))
